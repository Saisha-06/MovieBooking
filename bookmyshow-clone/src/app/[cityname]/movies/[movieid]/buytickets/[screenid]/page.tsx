...
const SelectSeatPage  = () => {
    ...
    const generateSeatLayout = () => {
        ...
        const x = screen.movieSchedulesforDate.findIndex((t: { showTime: any; }) => t.showTime === selectedTime.showTime);
    
        // Check if index is valid
        if (x === -1) {
            return <div>No schedule found for the selected time.</div>; // Handle case where schedule not found
        }
    
        // Access notAvailableSeats safely
        const notavailableseats = screen.movieSchedulesforDate[x].notAvailableSeats;


        return (
            <div>
                {screen.screen.seats.map((seatType: { type: string | number | bigint | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<React.AwaitedReactNode> | null | undefined; price: string | number | bigint | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<React.AwaitedReactNode> | null | undefined; rows: any[]; }, index: React.Key | null | undefined) => (
                            ...
                            {seatType.rows.map((row, rowIndex) => (
                                <div className="seat-row" key={rowIndex}>
                                    ...
                                    <div className="seat-cols">
                                        {row.cols.map((col: { seats: any[]; }, colIndex: React.Key | null | undefined) => (
