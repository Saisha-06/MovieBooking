...
router.delete('/deletebooking/:id', authTokenHandler, async (req, res, next) => {
    try {
        const bookingId = req.params.id;
        ...
        if (booking.userId.toString() !== userId) {
            return res.status(403).json(createResponse(false, 'Unauthorized to delete this booking', null));
        }
        ...
        // Optionally, update the Screen's movie schedule to remove the booked seats
        const screen = await Screen.findById(booking.screenId);
        if (screen) {
            const movieSchedule = screen.movieSchedules.find(schedule => 
                schedule.movieId.toString() === booking.movieId.toString() &&
                new Date(schedule.showDate).getTime() === new Date(booking.showDate).getTime() &&
                schedule.showTime === booking.showTime
            );
            
            if (movieSchedule) {
                movieSchedule.notAvailableSeats = movieSchedule.notAvailableSeats.filter(
                    seat => !booking.seats.some(bookedSeat => 
                        bookedSeat.row === seat.row && 
                        bookedSeat.col === seat.col && 
                        bookedSeat.seat_id === seat.seat_id
                    )
                );
                await screen.save();
            }
